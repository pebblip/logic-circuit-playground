...
import type { GateType } from '../../entities/types/gate';
  ...
  const categories: GateCategory[] = [
    {
      ...
      gates: [
        ...
        { type: 'AND', label: 'AND', icon: '🔵', locked: !availableGates.includes(GateType.AND) },
        { type: 'OR', label: 'OR', icon: '🟢', locked: !availableGates.includes(GateType.OR) },
        { type: 'NOT', label: 'NOT', icon: '🔴', locked: !availableGates.includes(GateType.NOT) },
        { type: 'XOR', label: 'XOR', icon: '🟡', locked: !availableGates.includes(GateType.XOR) },
      ],
    },
    {
      id: 'advanced',
      title: '演算ゲート',
      gates: [
        { type: 'NAND', label: 'NAND', icon: '🟣', locked: !availableGates.includes(GateType.NAND) },
        { type: 'NOR', label: 'NOR', icon: '🟠', locked: !availableGates.includes(GateType.NOR) },
        { type: 'XNOR', label: 'XNOR', icon: '🟤', locked: !availableGates.includes(GateType.XNOR) },
        { type: 'MUX', label: 'MUX', icon: '🔀', locked: !availableGates.includes(GateType.MUX_2TO1) },
        { type: 'DEMUX', label: 'DEMUX', icon: '🔁', locked: !availableGates.includes('DEMUX' as GateType) },
        { type: 'DECODER', label: 'DEC', icon: '📊', locked: !availableGates.includes('DECODER' as GateType) },
      ],
    },
    {
      id: 'memory',
      title: '記憶素子',
      gates: [
        { type: 'SR_LATCH', label: 'SR', icon: '💾', locked: !availableGates.includes(GateType.SR_LATCH) },
        { type: 'D_FLIP_FLOP', label: 'D-FF', icon: '📦', locked: !availableGates.includes(GateType.D_FLIP_FLOP) },
        { type: 'JK_FLIP_FLOP', label: 'JK-FF', icon: '📋', locked: !availableGates.includes('JK_FLIP_FLOP' as GateType) },
        { type: 'T_FLIP_FLOP', label: 'T-FF', icon: '🔄', locked: !availableGates.includes('T_FLIP_FLOP' as GateType) },
        { type: 'REGISTER', label: 'REG', icon: '📝', locked: !availableGates.includes(GateType.REGISTER_4BIT) },
        { type: 'CLOCK', label: 'CLK', icon: '⏰', locked: !availableGates.includes(GateType.CLOCK) },
      ],
    },
  ];
