...
import { DiscoveryModeSelector } from '../shared/ui/DiscoveryModeSelector';
import { DiscoveryNotification } from '../shared/ui/DiscoveryNotification';
...
// UI定数
import { PIN_CONSTANTS } from '../shared/config/ui';
...
const UltraModernCircuitWithViewModel: React.FC = () => {
  ...
  const startConnection = useCallback((gateId: string, pinType: 'input' | 'output', pinIndex: number, x: number, y: number) => {
    console.log('[DEBUG] startConnection called:', gateId, pinType, pinIndex);
  ...
  const completeConnection = useCallback((gateId: string, pinType: 'input' | 'output', pinIndex: number) => {
    console.log('[DEBUG] completeConnection called:', gateId, pinType, pinIndex);
    completeWireConnection(gateId, pinIndex, pinType === 'output');
                ...
                return (
                  <g key={`input-${index}`}>
                    ...
                    <circle 
                      ...
                      onMouseEnter={() => {
                        console.log('[DEBUG] Mouse entered input pin:', gate.id, index);
              ...
              (gate.type === 'AND' || gate.type === 'OR' || gate.type === 'NAND' || gate.type === 'NOR' || gate.type === 'XNOR' || gate.type === 'D_FLIP_FLOP' || gate.type === 'SR_LATCH') ? (
                <>
                  ...
                  <g>
                    ...
                    <circle 
                      ...
                      onMouseEnter={() => {
                        hoveredPinRef.current = { gateId: gate.id, pinType: 'input', pinIndex: 0 };
                      }}
                      onMouseLeave={() => {
                        hoveredPinRef.current = null;
                      }}
                    ...
                    <circle
                      ...
                      style={{ pointerEvents: 'none' }}
                      data-terminal="input"
                      data-testid={`gate-${gate.id}-input-0`}
                    />
                  </g>
                  ...
                  <g>
                    ...
                    <circle 
                      ...
                      onMouseEnter={() => {
                        hoveredPinRef.current = { gateId: gate.id, pinType: 'input', pinIndex: 1 };
                      }}
                      onMouseLeave={() => {
                        hoveredPinRef.current = null;
                      }}
                    ...
                    <circle
                      ...
                      style={{ pointerEvents: 'none' }}
                      data-terminal="input"
                      data-testid={`gate-${gate.id}-input-1`}
                    />
                  </g>
                </>
              ) : (
                <g>
                  ...
                  <circle 
                    ...
                    onMouseEnter={() => {
                      console.log('[DEBUG] Mouse entered single input pin:', gate.id);
                      hoveredPinRef.current = { gateId: gate.id, pinType: 'input', pinIndex: 0 };
                    }}
                    onMouseLeave={() => {
                      hoveredPinRef.current = null;
            ...
            {(gateType.isCustom && gate.outputs) || gate.type === 'D_FLIP_FLOP' || gate.type === 'SR_LATCH' || gate.type === 'REGISTER_4BIT' || gate.type === 'NUMBER_4BIT_INPUT' || gate.type === 'ADDER_4BIT' ? (
              ...
              <g>
                ...
                <circle
                  ...
                  onMouseEnter={() => {
                    console.log('[DEBUG] Mouse entered standard output pin:', gate.id);
                    hoveredPinRef.current = { gateId: gate.id, pinType: 'output', pinIndex: 0 };
                  }}
                  onMouseLeave={() => {
                    hoveredPinRef.current = null;
                  }}
      ...
      {showCustomGatePanel && Object.keys(customGates).length > 0 && (
        <div 
          ...
          <svg
            ...
            onClick={(e) => {
              // SVGクリックでのゲート配置を無効化（ワンクリック配置に統一）
              // 将来的に、配線の開始/終了などの機能をここに追加できる
            }}
