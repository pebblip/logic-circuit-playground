...
import { DrawingWire } from '../entities/types/circuit';
...
export function useWireDrawing({ viewModel }: UseWireDrawingProps): UseWireDrawingReturn {
  ...
  const completeWireConnection = useCallback((gateId: string, pinIndex: number, isOutput: boolean) => {
    console.log('[DEBUG] completeWireConnection in hook:', { gateId, pinIndex, isOutput, drawingWire });
    if (drawingWire) {
      const { from, fromOutput, fromInput, pinType } = drawingWire;
      
      if (pinType === 'output' && !isOutput) {
        // 出力から入力へ
        console.log('[DEBUG] Connecting output to input:', from, fromOutput || 0, gateId, pinIndex);
        viewModel.addConnection(from, fromOutput || 0, gateId, pinIndex);
      } else if (pinType === 'input' && isOutput) {
        // 入力から出力へ
        console.log('[DEBUG] Connecting input to output:', gateId, pinIndex, from, fromInput || 0);
        viewModel.addConnection(gateId, pinIndex, from, fromInput || 0);
      } else {
        console.log('[DEBUG] Invalid connection attempt:', pinType, isOutput);
      }
    } else {
      console.log('[DEBUG] No drawing wire!');
    }
