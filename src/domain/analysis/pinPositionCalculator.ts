 ...
 */
export function getOutputPinPosition(gate: Gate, pinIndex: number = 0): Position {
  const { x, y } = gate.position;
...
export function getAllPinPositions(gate: Gate): PinPosition[] {
  ...
  // 入力ピン
  const inputCount = gate.type === 'CUSTOM' && gate.customGateDefinition
  ...
  // 出力ピン
  const outputCount = gate.type === 'CUSTOM' && gate.customGateDefinition
  ...
  for (const gate of gates) {
    ...
    for (const pin of pins) {
      ...
      const distance = Math.sqrt(
        Math.pow(position.x - pin.x, 2) + 
        Math.pow(position.y - pin.y, 2)
      );
