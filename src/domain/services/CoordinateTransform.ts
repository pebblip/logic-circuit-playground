...
export class CoordinateTransform {
  ...
  /**
   * キャンバス座標をSVG座標に変換（高精度）
   * SVGネイティブのgetScreenCTM()を使用
   * canvasPointは画面上の絶対座標（clientX, clientY）
   */
  canvasToSvg(canvasPoint: Position, svgElement: SVGSVGElement): Position {
    try {
      ...
      // SVG要素内の相対座標に変換
      const relativeX = canvasPoint.x - rect.left;
      const relativeY = canvasPoint.y - rect.top;
      ...
      if (!matrix) {
        // フォールバック: 手動計算
        return this.fallbackCanvasToSvg(canvasPoint, svgElement);
      }
      ...
      return {
        x: Math.round(transformed.x * 1000) / 1000, // 小数点3桁で丸め
