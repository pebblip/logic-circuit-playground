...
export class ConnectionValidator {
  ...
  validateConnection(
    ...
    // 3. ピンのインデックスチェック
    if (fromPinIndex < 0 || fromPinIndex >= fromGate.outputPins.length) {
      return { isValid: false, reason: '出力ピンのインデックスが無効です' };
    }
    
    if (toPinIndex < 0 || toPinIndex >= toGate.inputPins.length) {
  ...
  getConnectablePins(
    ...
    // 入力ピンのみ接続可能
    for (let i = 0; i < targetGate.inputPins.length; i++) {
