...
export function isLFSRCircuit(circuit: Circuit): boolean {
  ...
  const hasXORFeedback = circuit.gates.some(
    g => g.id === 'xor_feedback' && g.type === 'XOR'
  );
  const hasDFFChain =
    circuit.gates.filter(g => g.type === 'D-FF' && g.id.startsWith('dff'))
      .length >= 2;
  ...
  const feedbackWire = circuit.wires.find(
    w => w.from.gateId === 'xor_feedback' && w.to.gateId.startsWith('dff')
  );
...
export function validateLFSRInitialState(circuit: Circuit): boolean {
  const dffGates = circuit.gates.filter(
    g => g.type === 'D-FF' && g.id.startsWith('dff')
  );
  const hasNonZero = dffGates.some(
    g => g.output === true || g.metadata?.qOutput === true
  );

  if (!hasNonZero) {
    console.warn(
      '[LFSR] Warning: All D-FFs are initialized to 0. LFSR will not function properly.'
    );
    return false;
  }
