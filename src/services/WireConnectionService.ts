...
export class WireConnectionService {
  ...
  static canConnect(
    ...
    const isDuplicate = existingWires.some(
      wire =>
        (wire.from.gateId === fromGate.id &&
          wire.from.pinIndex === fromPinIndex &&
          wire.to.gateId === toGate.id &&
          wire.to.pinIndex === toPinIndex) ||
        (wire.from.gateId === toGate.id &&
          wire.from.pinIndex === toPinIndex &&
          wire.to.gateId === fromGate.id &&
          wire.to.pinIndex === fromPinIndex)
    );
    ...
    if (!toIsOutput) {
      const hasExistingInput = existingWires.some(
        wire => wire.to.gateId === toGate.id && wire.to.pinIndex === toPinIndex
      );
      if (hasExistingInput) {
        return {
          valid: false,
          reason: '入力ピンには1つまでしか接続できません',
        };
      }
    }
  ...
  static removeWiresConnectedToGates(gateIds: string[], wires: Wire[]): Wire[] {
    ...
    return wires.filter(
      wire => !gateIdSet.has(wire.from.gateId) && !gateIdSet.has(wire.to.gateId)
    );
  }
}
