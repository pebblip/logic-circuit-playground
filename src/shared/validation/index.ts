...
export class ValidationRules {
   ...
   * @returns バリデーション関数
   */
  static length(min: number = 0, max: number = Infinity, fieldName: string = 'この項目'): ValidationFunction<string> {
    return (value: string): ValidationResult => {
      const trimmed = value?.trim() || '';
  ...
  static gateName(): ValidationFunction<string> {
    return (value: string): ValidationResult => {
      ...
      if (!validPattern.test(trimmed)) {
        return {
          isValid: false,
          error: 'ゲート名は英字で始まり、英数字とアンダースコアのみ使用できます'
        };
      }
   ...
   * ファイル名のバリデーション
   */
  static fileName(allowedExtensions: string[] = []): ValidationFunction<string> {
    return (value: string): ValidationResult => {
      const trimmed = value?.trim() || '';
   ...
   * 数値のバリデーション
   */
  static number(min: number = -Infinity, max: number = Infinity): ValidationFunction<string | number> {
 ...
 * 複数のバリデーションを組み合わせる
 */
export function combineValidations<T>(...validators: ValidationFunction<T>[]): ValidationFunction<T> {
...
export class FormValidator {
   ...
   * 全フィールドのバリデーション
   */
  validateAll(): { isValid: boolean; errors: Record<string, string>; values: Record<string, any> } {
    this.errors = {};
 ...
 * 単一値の即座なバリデーション
 */
export function validate<T>(value: T, validator: ValidationFunction<T>): ValidationResult {
