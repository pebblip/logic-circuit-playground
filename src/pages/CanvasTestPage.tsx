...
export const CanvasTestPage: React.FC = () => {
  ...
  const handlers = useMemo(
    (): CanvasEventHandlers => ({
      ...
      onCanvasClick: position => {
        addLog(
          `キャンバスクリック: (${Math.round(position.x)}, ${Math.round(position.y)})`
        );
      },
      onZoomChange: scale => {
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ color: '#00ff88', marginBottom: '10px' }}>
          統一キャンバステストページ
        </h1>
      ...
      >
        ...
        >
          <h3
            style={{ color: '#00ff88', marginBottom: '10px', fontSize: '16px' }}
          >
            動作ログ
          </h3>
          <div
          ...
          >
            ...
            <div style={{ fontSize: '11px', color: '#999' }}>
              ...
              <div>
                コントロール: {config.uiControls?.showControls ? 'ON' : 'OFF'}
              </div>
              <div>
                自動シミュレーション:{' '}
                {config.galleryOptions?.autoSimulation ? 'ON' : 'OFF'}
              </div>
            </div>
          </div>
        </div>
