...
export const useCircuitStore = create<CircuitStore>((set, get) => ({
          ...
          if (gate.type === 'INPUT') {
            ...
            if (isOutput) {
              ...
              const pinY =
                pinCount === 1
                  ? 0
                  : -((pinCount - 1) * spacing) / 2 + outputIndex * spacing;
              ...
              const pinY =
                pinCount === 1
                  ? 0
                  : -((pinCount - 1) * spacing) / 2 + pinIndex * spacing;
  ...
  endWireDrawing: (gateId, pinIndex) => {
    set(state => {
      ...
      const targetInputPin =
        to.pinIndex >= 0 ? to : from.pinIndex >= 0 ? from : null;
      if (targetInputPin) {
        const existingConnection = state.wires.find(
          wire =>
  ...
  copySelection: () => {
    const state = get();
    const selectedGates = state.gates.filter(g =>
      state.selectedGateIds.includes(g.id)
    );
  ...
  paste: position => {
    ...
    const newGates = clipboardGates.map(gate => {
      ...
      // ペースト時に入力値の形式を統一
      const normalizedInputs = gate.inputs
        ? gate.inputs.map(input =>
            typeof input === 'boolean' ? booleanToDisplayState(input) : input
          )
        : [];
  ...
  canPaste: () =>
    get().clipboard !== null && (get().clipboard?.gates.length ?? 0) > 0,
