...
import type { SavedCircuit, CircuitMetadata } from '@/types/circuitStorage';
...
export class ServiceCircuitStorageAdapter implements CircuitStorage {
  ...
  private convertFromServiceFormat(savedCircuit: SavedCircuit): CircuitContent {
    return {
  ...
  private convertMetadata(metadata: CircuitMetadata): {
    id: CircuitId;
    name: string;
    updatedAt: Date;
  } {
  ...
  async save(content: CircuitContent): Promise<CircuitId> {
    const { name, gates, wires, options } = this.convertToServiceFormat(content);
    
    const result = await this.storageService.saveCircuit(name, gates, wires, options);
  ...
  async list(): Promise<Array<{ id: CircuitId; name: string; updatedAt: Date }>> {
  ...
  async loadFromShareUrl(shareUrl: ShareUrl): Promise<CircuitContent> {
    try {
      ...
      const result = await CircuitShareService.loadFromUrl(shareUrl);
      ...
      return {
        ...
        components: result.data.gates?.map(gate => ({
          id: gate.id,
          type: gate.type,
          position: gate.position,
        })) || [],
        connections: result.data.wires?.map(wire => ({
          id: wire.id,
          from: {
            componentId: wire.from.gateId,
            outputIndex: wire.from.pinIndex,
      ...
      throw new Error(`無効な共有URL: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  async validate(content: CircuitContent): Promise<ValidationResult> {
    const errors: Array<{ type: string; message: string; componentId?: string }> = [];
    ...
    return {
      ...
      errors,
      warnings,
      canRepair: errors.some(e => e.type === 'MISSING_COMPONENT' || e.type === 'INVALID_NAME'),
    };
  }
  ...
  async repair(content: CircuitContent): Promise<CircuitContent> {
    ...
    repaired.connections = repaired.connections.filter(connection =>
  ...
  async exportToFile(content: CircuitContent, format: 'json' | 'png'): Promise<Blob> {
    if (format === 'json') {
  ...
  async importFromFile(file: File): Promise<CircuitContent> {
    try {
      ...
      // 基本的なフォーマット検証
      if (!parsed.name || !Array.isArray(parsed.components) || !Array.isArray(parsed.connections)) {
