...
export const BinaryExpressionRenderer: React.FC<
  BinaryExpressionRendererProps
> = ({ content }) => {
  // 単一の式か複数の式かを判定
  const expressions =
    'expressions' in content ? content.expressions : [content];
  ...
  const renderExpression = (
    ...
    isExperimentResult = false
  ) => {
    const className = isExperimentResult
      ? 'experiment-result'
      : 'binary-expression';
    ...
    return (
      <span className={className}>
        ...
        <span className={operatorClass}> {formatOperator(expr.operator)} </span>
