...
import { Puzzle, PuzzleTestCase } from '../data/puzzles';
...
export class PuzzleValidator {
  ...
  private validateConstraints(gates: Gate[], wires: Wire[]): string[] {
    ...
    if (constraints.maxGates && gates.length > constraints.maxGates) {
      errors.push(`ゲート数が上限を超えています (${gates.length}/${constraints.maxGates})`);
    }

    // ワイヤー数制限
    if (constraints.maxWires && wires.length > constraints.maxWires) {
      errors.push(`ワイヤー数が上限を超えています (${wires.length}/${constraints.maxWires})`);
    }

    // 使用可能ゲート制限
    const usedGateTypes = gates.map(g => g.type);
    const disallowedGates = usedGateTypes.filter(type => 
      !constraints.allowedGates.includes(type)
    );
    if (disallowedGates.length > 0) {
      errors.push(`使用不可のゲート: ${[...new Set(disallowedGates)].join(', ')}`);
    }

    // 必須ゲート確認
    if (constraints.requiredGates) {
      const missingGates = constraints.requiredGates.filter(required =>
        !usedGateTypes.includes(required)
      );
   ...
   */
  private runSingleTest(gates: Gate[], wires: Wire[], testCase: PuzzleTestCase): boolean {
    try {
      // 1. 入力ゲートを特定
      const inputGates = gates.filter(g => g.type === 'INPUT');
      const outputGates = gates.filter(g => g.type === 'OUTPUT');
      ...
      if (inputGates.length !== testCase.inputs.length) {
        console.warn(`入力数が一致しません: ${inputGates.length} vs ${testCase.inputs.length}`);
        return false;
      }
  ...
  private simulateCircuit(gates: Gate[], wires: Wire[]): Gate[] {
    ...
    result.filter(g => g.type === 'INPUT' || g.type === 'CLOCK')
    ...
    for (let iteration = 0; iteration < 10; iteration++) {
      ...
      result.forEach(gate => {
        ...
        if (gate.type === 'INPUT' || gate.type === 'CLOCK' || gate.type === 'OUTPUT') return;
      ...
      // OUTPUTゲートの更新
      result.filter(g => g.type === 'OUTPUT').forEach(outputGate => {
   ...
   * ゲートの入力値を取得
   */
  private getGateInputs(gate: Gate, gates: Gate[], wires: Wire[]): boolean[] | null {
