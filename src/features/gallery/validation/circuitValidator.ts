import type { CircuitMetadata, SimulationConfig } from '../data/gallery';
...
export class CircuitValidator {
  ...
  private static validateBasicStructure(circuit: CircuitMetadata, errors: ValidationError[]) {
    if (!circuit.id) {
      errors.push({
        type: 'error',
  ...
  private static validateSimulationConfig(
    ...
    const hasOscillatorKeywords = [
      'オシレータ', 'カオス', 'メモリ', 'フィボナッチ', 'マンダラ', 'ジョンソン'
      ...
      if (!circuit.simulationConfig?.needsAnimation) {
        errors.push({
          ...
          autoFix: () => {
            ...
            circuit.simulationConfig!.updateInterval = circuit.simulationConfig!.updateInterval ?? 200;
            circuit.simulationConfig!.expectedBehavior = circuit.simulationConfig!.expectedBehavior ?? 'oscillator';
          }
        });
      }
    }
    ...
    if (clockGates.length > 0 && circuit.simulationConfig?.clockFrequency) {
      clockGates.forEach(clock => {
        if (clock.metadata?.frequency !== circuit.simulationConfig?.clockFrequency) {
          warnings.push({
            type: 'warning',
  ...
  private static validateMetadataNaming(circuit: CircuitMetadata, errors: ValidationError[]) {
  ...
  private static validateOscillatorConsistency(circuit: CircuitMetadata, warnings: ValidationError[]) {
    if (circuit.simulationConfig?.needsAnimation) {
      ...
      if (outputGates.length === 0) {
        warnings.push({
          type: 'warning',
          message: 'オシレーター回路ですが OUTPUT ゲートがありません。動作確認が困難です。'
        });
      }
  ...
  private static validatePerformance(circuit: CircuitMetadata, warnings: ValidationError[]) {
