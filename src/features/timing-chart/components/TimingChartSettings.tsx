...
export const TimingChartSettings: React.FC<TimingChartSettingsProps> = ({
  ...
  // 個別設定の更新ハンドラー
  const updateSetting = useCallback(<K extends keyof SettingsType>(
    key: K, 
  ...
  // プリセット設定
  const applyPreset = useCallback((preset: 'performance' | 'quality' | 'default') => {
    switch (preset) {
      case 'performance':
        onSettingsChange({
  ...
  return (
    <div className={`timing-chart-settings p-4 bg-gray-800 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-sm font-semibold text-white">タイミングチャート設定</h4>
        <button
          onClick={onClose}
      ...
      <div className="mt-6 pt-4 border-t border-gray-700">
        ...
        <div className="grid grid-cols-3 gap-4 text-xs text-gray-400">
          ...
          <div>
            ...
            <br />
            ~{Math.round(settings.captureDepth * 200 / 1024)} KB
          </div>
          <div>
...
// スタイルの注入
if (typeof window !== 'undefined' && !document.querySelector('#timing-chart-settings-styles')) {
