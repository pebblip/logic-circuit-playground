...
export const useCanvasGateManagement = ({ 
  ...
  const handleDrop = useCallback((event: React.DragEvent) => {
    ...
    if (!svgPoint) {
      console.warn('⚠️ SVG座標変換に失敗しました。フォールバック座標を使用します。');
      ...
      if (draggedGate.type === 'CUSTOM' && draggedGate.customDefinition) {
        ...
        console.log('🎯 ゲート配置実行:', { type: draggedGate.type, position: { x: safeX, y: safeY } });
    ...
    // ゲートを配置（安全な座標で）
    if (draggedGate.type === 'CUSTOM' && draggedGate.customDefinition) {
      console.log('🎯 カスタムゲート配置実行:', { type: draggedGate.type, position: { x: safeX, y: safeY } });
      addCustomGateInstance(draggedGate.customDefinition, {
        x: safeX,
        y: safeY,
      });
    } else {
      console.log('🎯 標準ゲート配置実行:', { type: draggedGate.type, position: { x: safeX, y: safeY } });
      addGate(draggedGate.type, { x: safeX, y: safeY });
    }
