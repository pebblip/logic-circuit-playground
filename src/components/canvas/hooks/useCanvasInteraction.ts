...
interface UseCanvasInteractionProps {
  ...
  setInitialGatePositions: (value: Map<string, { x: number; y: number }>) => void;
...
export const useCanvasInteraction = ({
  ...
  const isClickOnSelectedGate = useCallback((x: number, y: number): boolean => {
  ...
  // クリック位置が選択矩形内にあるかを判定
  const isClickInSelectionRect = useCallback((x: number, y: number): boolean => {
  ...
  }, [svgRef, isPanning, isSelecting, setMousePosition, handlePan, updateSelection]);
