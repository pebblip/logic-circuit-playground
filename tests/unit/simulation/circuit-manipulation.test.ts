...
describe('回路操作機能', () => {
  ...
  describe('ゲートの追加と削除', () => {
    test('ゲートが正しい位置に追加される', () => {
      ...
      // Gate inputs are initialized as boolean values in the current implementation
      expect(gates[0].inputs).toEqual([false, false]);
  ...
  describe('回路のコピー・ペースト', () => {
    test('選択したゲートをコピー・ペーストできる', () => {
      ...
      // 位置がオフセットされている
      expect(pastedGates[0].position.x).toBeGreaterThan(
        originalGates[0].position.x
      );
      expect(pastedGates[0].position.y).toBeGreaterThan(
        originalGates[0].position.y
      );
    });
