...
describe('🔬 chaos-generator美的メトリクス検証', () => {
  it('✨ chaos-generator美的改善後の詳細メトリクス', () => {
    const chaosCircuit = FEATURED_CIRCUITS.find(c => c.id === 'chaos-generator');
    if (!chaosCircuit) return;
    
    console.log('\n🔬 chaos-generator美的メトリクス検証');
    console.log(`📊 総ゲート数: ${chaosCircuit.gates.length}`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標:');
    chaosCircuit.gates.forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(chaosCircuit.gates);
    ...
    // 改善度評価
    const originalScore = 79.0; // 改善前のスコア
