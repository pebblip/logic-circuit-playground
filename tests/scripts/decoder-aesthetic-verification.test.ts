...
function calculateActualAestheticMetrics(gates: Gate[]) {
  ...
  // 5. 間隔の最適性
  const targetSpacing = 50; // decoderは50px間隔が適切
  let spacingScore = 0;
  if (rowSpacings.length > 0) {
...
describe('🔬 decoder美的メトリクス検証', () => {
  it('✨ decoder美的改善後の詳細メトリクス', () => {
    const decoderCircuit = FEATURED_CIRCUITS.find(c => c.id === 'decoder');
    if (!decoderCircuit) return;
    
    console.log('\n🔬 decoder美的メトリクス検証');
    console.log(`📊 総ゲート数: ${decoderCircuit.gates.length}`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標:');
    decoderCircuit.gates.forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(decoderCircuit.gates);
    ...
    // 改善度評価
    const originalScore = 56.6; // 改善前のスコア
    ...
    if (metrics.overall >= 76) {
      console.log(`  🌟 76点台達成成功！予想通り！`);
