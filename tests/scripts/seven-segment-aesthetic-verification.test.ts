...
function calculateActualAestheticMetrics(gates: Gate[]) {
  ...
  // 5. 間隔の最適性
  const targetSpacing = 50; // seven-segmentは50px間隔が適切
  let spacingScore = 0;
  if (rowSpacings.length > 0) {
...
describe('🔬 seven-segment美的メトリクス検証', () => {
  it('✨ seven-segment美的改善後の詳細メトリクス', () => {
    const sevenSegmentCircuit = FEATURED_CIRCUITS.find(c => c.id === 'seven-segment');
    if (!sevenSegmentCircuit) return;
    
    console.log('\n🔬 seven-segment美的メトリクス検証');
    console.log(`📊 総ゲート数: ${sevenSegmentCircuit.gates.length}`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標:');
    sevenSegmentCircuit.gates.forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(sevenSegmentCircuit.gates);
    ...
    // 改善度評価
    const originalScore = 53.6; // 改善前のスコア
    ...
    if (metrics.overall >= 73) {
      console.log(`  🌟 73点台達成成功！予想通り！`);
