/**
 * self-oscillating-memory-simple美的メトリクス検証
 */
...
function calculateActualAestheticMetrics(gates: Gate[]) {
  ...
  if (inputGates.length > 1) {
    ...
    // 単一入力の場合はフル点数
    metrics.symmetry += 50;
    console.log(`    入力対称性: 単一入力, スコア: 50.0`);
  }
  ...
  // 5. 間隔の最適性
  const targetSpacing = 50; // self-oscillating-memory-simpleは50px間隔が適切
  let spacingScore = 0;
  if (rowSpacings.length > 0) {
  ...
  console.log(`    間隔最適性: ${rowSpacings.filter(s => Math.abs(s - targetSpacing) <= 10).length}/${rowSpacings.length}個が50px±10px (${spacingScore.toFixed(1)}%)`);
...
describe('🔬 self-oscillating-memory-simple美的メトリクス検証', () => {
  it('✨ self-oscillating-memory-simple美的改善後の詳細メトリクス', () => {
    const oscillatingCircuit = FEATURED_CIRCUITS.find(c => c.id === 'self-oscillating-memory-simple');
    if (!oscillatingCircuit) return;
    
    console.log('\n🔬 self-oscillating-memory-simple美的メトリクス検証');
    console.log(`📊 総ゲート数: ${oscillatingCircuit.gates.length}`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標:');
    oscillatingCircuit.gates.forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(oscillatingCircuit.gates);
    ...
    // 改善度評価
    const originalScore = 54.3; // 改善前のスコア
    ...
    if (metrics.overall >= 74) {
      console.log(`  🌟 74点台達成成功！予想通り！`);
    } else if (metrics.overall >= 70) {
      console.log(`  🎉 70点台達成成功！`);
