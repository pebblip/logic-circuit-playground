...
describe('🔬 johnson-counter美的メトリクス検証', () => {
  it('✨ johnson-counter美的改善後の詳細メトリクス', () => {
    ...
    // 重なりチェック
    const overlaps = checkOverlaps(johnsonCircuit.gates);
    console.log(`重なり: ${overlaps.length}件`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標（最初の10個）:');
    johnsonCircuit.gates.slice(0, 10).forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(johnsonCircuit.gates);
    ...
    if (comparatorCircuit) {
      ...
      console.log(`\n📊 4bit-comparator比較:`);
      console.log(`  4bit-comparator総合スコア: ${comparatorMetrics.overall.toFixed(1)}/100`);
      console.log(`  johnson-counter総合スコア: ${metrics.overall.toFixed(1)}/100`);
      const improvement = metrics.overall - comparatorMetrics.overall;
      console.log(`  スコア差: ${improvement > 0 ? '+' : ''}${improvement.toFixed(1)}点`);
    }
  });
});