...
describe('🔬 sr-latch-basic美的メトリクス検証', () => {
  it('✨ sr-latch-basic美的改善後の詳細メトリクス', () => {
    const srLatchCircuit = FEATURED_CIRCUITS.find(c => c.id === 'sr-latch-basic');
    if (!srLatchCircuit) return;
    
    console.log('\n🔬 sr-latch-basic美的メトリクス検証');
    console.log(`📊 総ゲート数: ${srLatchCircuit.gates.length}`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標:');
    srLatchCircuit.gates.forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(srLatchCircuit.gates);
    ...
    // 改善度評価
    const originalScore = 74.6; // 改善前のスコア
