...
function calculateActualAestheticMetrics(gates: Gate[]) {
  ...
  // 5. 間隔の最適性
  const targetSpacing = 50; // parity-checkerは50px間隔が適切
  let spacingScore = 0;
  if (rowSpacings.length > 0) {
...
describe('🔬 parity-checker美的メトリクス検証', () => {
  it('✨ parity-checker美的改善後の詳細メトリクス', () => {
    const parityCircuit = FEATURED_CIRCUITS.find(c => c.id === 'parity-checker');
    if (!parityCircuit) return;
    
    console.log('\n🔬 parity-checker美的メトリクス検証');
    console.log(`📊 総ゲート数: ${parityCircuit.gates.length}`);
    
    // 実際の座標出力（デバッグ用）
    console.log('\n📍 実際の座標:');
    parityCircuit.gates.forEach(gate => {
      console.log(`    ${gate.id}: (${gate.position.x}, ${gate.position.y}) // ${gate.type}`);
    });
    
    // 美的メトリクス計算
    console.log('\n📊 詳細美的メトリクス分析:');
    const metrics = calculateActualAestheticMetrics(parityCircuit.gates);
    ...
    // 改善度評価
    const originalScore = 67.2; // 改善前のスコア
    ...
    if (metrics.overall >= 87) {
      console.log(`  🌟 87点台達成成功！予想通り！`);
