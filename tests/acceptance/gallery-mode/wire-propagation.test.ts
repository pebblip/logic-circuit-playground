...
describe('Wire Propagation and OUTPUT Gate Analysis', () => {
  ...
  it('should analyze initial circuit structure and connections', () => {
    ...
    outputGates.forEach(outputGate => {
      ...
      if (inputWire) {
        console.log(
          `${outputGate.id} ← ${inputWire.from.gateId} (wire: ${inputWire.id})`
        );
      } else {
  ...
  it('should check initial input values of OUTPUT gates', () => {
    ...
    outputGates.forEach(outputGate => {
      ...
      console.log(
        `  - inputs[0]: "${outputGate.inputs[0]}" (type: ${typeof outputGate.inputs[0]})`
      );
  ...
  it('should trace simulation steps and wire propagation', () => {
    ...
    for (let step = 0; step < 5; step++) {
      ...
      notGates.forEach(gate => {
        console.log(
          `  ${gate.id}: output=${gate.output}, inputs=[${gate.inputs.join(', ')}]`
        );
      });
      ...
      outputGates.forEach(gate => {
        ...
        console.log(
          `  ${gate.id}: inputs[0]="${gate.inputs[0]}" → inputValue=${inputValue}, output=${gate.output}`
        );
      });

      // シミュレーション実行
      const result = evaluator.evaluateCircuit(currentCircuit);
      currentCircuit = result.circuit;

      // OUTPUTゲートの状態（シミュレーション後）
      const outputGatesAfter = currentCircuit.gates.filter(
        g => g.type === 'OUTPUT'
      );
  ...
  it('should verify wire propagation logic', () => {
    ...
    // NOT1に接続されているワイヤーを探す
    const wiresFromNot1 = testCircuit.wires.filter(
      w => w.from.gateId === 'NOT1'
    );
    console.log(
      `Wires from NOT1: [${wiresFromNot1.map(w => w.id).join(', ')}]`
    );
    ...
    wiresFromNot1.forEach(wire => {
      ...
      if (targetGate) {
        console.log(
          `Wire ${wire.id}: NOT1 → ${wire.to.gateId}[${wire.to.pinIndex}]`
        );
        console.log(
          `  Target gate before: inputs=[${targetGate.inputs.join(', ')}]`
        );
        ...
        if (wire.to.pinIndex < targetGate.inputs.length) {
          ...
          console.log(
            `  Target gate after: inputs=[${targetGate.inputs.join(', ')}]`
          );
