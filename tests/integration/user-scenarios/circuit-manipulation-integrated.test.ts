...
import { ZustandCircuitAdapter } from '@/adapters/ZustandCircuitAdapter';
import { useCircuitStore } from '@/stores/circuitStore';
import type { Circuit, ComponentId } from '@/domain/ports/CircuitDesigner';

describe('🚀 回路設計者として（統合テスト：理想 × Zustand）', () => {
  let circuit: Circuit;
  ...
  beforeEach(() => {
    ...
    // 🔥 理想インターフェース × 実システムの統合
    circuit = new ZustandCircuitAdapter();
  });
  ...
  describe('⚡ 基本的なゲート操作', () => {
    ...
    test('存在しないゲートの削除は適切にエラーになる', async () => {
      ...
      await expect(circuit.remove('non-existent-id'))
        .rejects.toThrow('削除対象のゲートが見つかりません');
    });
  });
  ...
  describe('🔗 実システムでの接続操作', () => {
    ...
    test('無効な接続は適切にエラーになる', async () => {
      ...
      await expect(circuit.connect(gateId, gateId))
        .rejects.toThrow('接続できません');
      
      // When & Then: 存在しないゲートとの接続はエラー
      await expect(circuit.connect(gateId, 'non-existent'))
        .rejects.toThrow('接続できません');
    });
  ...
  describe('⏪ 実システムでのUndo/Redo', () => {
    ...
    test('Undoできない状態での操作は適切にエラー', async () => {
      ...
      // When & Then: Undoはエラー
      await expect(circuit.undo()).rejects.toThrow('取り消せる操作がありません');
    });
    ...
    test('Redoできない状態での操作は適切にエラー', async () => {
      ...
      // When & Then: Redoはエラー
      await expect(circuit.redo()).rejects.toThrow('やり直せる操作がありません');
    });
  });
