  ...
  subscribeWithSelector(
    immer((set, get) => ({
      ...
      startConnection: (gateId: string, pinIndex: number, pinType: 'input' | 'output') => {
        console.log('[circuitStore] startConnection called:', { gateId, pinIndex, pinType });
        
        // 出力ピンからのみ接続開始可能
        if (pinType !== 'output') {
          console.log('[circuitStore] startConnection failed: not an output pin');
          set((draft) => {
        ...
        // 開始位置を計算
        const fromPos = state.collisionDetector.calculateOutputPinPosition(gate, pinIndex);
        
        console.log('[circuitStore] Starting connection from position:', fromPos);
        console.log('[circuitStore] Connectable pins:', Array.from(connectablePins.entries()));
        ...
        set((draft) => {
          draft.drawingConnection = {
            ...
            fromPinType: pinType,
            connectablePins,
            currentPosition: fromPos // 初期位置を設定
          };
          draft.lastConnectionError = null;
        });

        console.log('[circuitStore] drawingConnection state set successfully');
        return true;
      },

      updateConnectionPosition: (position: Position) => {
        console.log('[circuitStore] updateConnectionPosition called:', position);
        set((draft) => {
          if (draft.drawingConnection) {
            draft.drawingConnection.currentPosition = position;
            console.log('[circuitStore] Updated drawing position');
          } else {
            console.log('[circuitStore] No active drawingConnection to update');
          }
        });
      },
