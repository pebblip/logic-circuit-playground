...
export class CollisionDetector {
  ...
  calculateInputPinPosition(gate: BaseGate, pinIndex: number): Position {
    ...
    if (pinCount === 1) {
      ...
      y = gate.position.y + (pinIndex === 0 ? -10 : 10);
    } else if (pinCount === 3) {
      y = gate.position.y + (pinIndex - 1) * 20;
    } else {
      const spacing = 50 / (pinCount + 1);
      y = gate.position.y - 25 + spacing * (pinIndex + 1);
    }
    
    // モックアップ準拠の固定X座標
    let x = 0;
    if (gate.type === 'OUTPUT') {
      x = gate.position.x - 30; // OUTPUTゲートの入力ピンは30px左
    } else {
      x = gate.position.x - 45; // その他のゲートの入力ピンは45px左
    }
  ...
  calculateOutputPinPosition(gate: BaseGate, pinIndex: number): Position {
    ...
    switch (gate.type) {
      case 'INPUT':
        x = gate.position.x + 35; // INPUTは35px右
        break;
      case 'OUTPUT':
        x = gate.position.x - 30; // OUTPUTは入力ピンなので左側
        break;
      default:
        x = gate.position.x + 45; // その他は45px右
        break;
    }
