describe('新しいビジュアライザーテスト', () => {
  ...
  it('新しいビジュアライザーの基本機能をテスト', () => {
    ...
    cy.get('[data-testid="circuit-monitor-title"]').should('be.visible');
    cy.log('✅ 新しいタイトル「回路モニター」が表示されています');
    
    // 3. 初期状態では「動作状況が表示されます」が表示
    cy.get('[data-testid="circuit-monitor-empty-message"]').should('be.visible');
    ...
    cy.get('[data-testid="circuit-monitor-title"]').should('not.exist');
